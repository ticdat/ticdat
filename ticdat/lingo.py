...
def lingo_run(lng_file, input_tdf, input_dat, soln_tdf, infinity=INFINITY, lingorun_path=None, post_solve=lambda k: k):
    ...
    with open(lng_file, "r") as f:
        lng = f.read()
        assert ("ticdat_" + lng_file_name + ".mod") in lng
    ...
    with open(os.path.join(working_dir, "ticdat_"+lng_file_name+".mod"), "w") as f:
    ...
    return post_solve(read_lingo_text(soln_tdf, output_data))
...
def create_lingo_mod_text(tdf):
    ...
    sets:
    foods: foods_cost;
    categories: categories_min_nutrition, categories_max_nutrition;
    nutrition_quantities(foods, categories) : nutrition_quantities_quantity;
    endsets
    """
    """
    BAD
    ! Autogenerated input file, created by lingo.py on Fri May 12 23:42:13 2017 ;
    sets:
    foods:foods_cost;
    categories:categories_min_nutrition,categories_max_nutrition;
    nutrition_quantities(foods, categories) :nutrition_quantities_quantity;
    ...
    def get_table_as_mod_text(tdf, tbn):
        ...
        if len(tdf.primary_key_fields[tbn]) > 1:
            rtn += '(foods, categories) ' # I'm cheating
        rtn += ':'
        fields = []
