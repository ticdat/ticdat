...
def opl_run(mod_file, input_tdf, input_dat, soln_tdf, infinity=INFINITY, oplrun_path=None, post_solve=None, debug=False):
    """
    ...
    with open("ticdat_"+mod_file, "w") as f:
        f.write("/* Autogenerated file, created by opl.py on " + time.asctime() + "*/\n")
        f.write(create_opl_mod_text(input_tdf, soln_tdf))
    if not oplrun_path:
    ...
    if post_solve:
        post_solve()
    if debug:
        with open("debug-oplrun-output.txt","w") as f:
            f.write(output)
    min = len(output) + 1
