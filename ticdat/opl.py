...
def opl_run(mod_file, input_tdf, input_dat, soln_tdf, infinity=INFINITY, oplrun_path=None, post_solve=None):
    ...
    with open(os.path.join(os.path.dirname(mod_file),"ticdat_"+mod_file_name+".mod"), "w") as f:
        f.write("/* Autogenerated input file, created by opl.py on " + time.asctime() + " */\n")
        f.write(create_opl_mod_text(input_tdf, False))
    with open(os.path.join(os.path.dirname(mod_file),"ticdat_"+mod_file_name+"_output.mod"), "w") as f:
        f.write("/* Autogenerated output file, created by opl.py on " + time.asctime() + " */\n")
        f.write(create_opl_mod_text(soln_tdf, True))
    if not oplrun_path:
    ...
    verify(os.path.isfile("results.txt"), "results.txt is not a valid file.")
    with open("results.txt", "r") as f:
        output = f.read()
    if post_solve:
        post_solve()
    # What happens if a solution is infeasible
...
def create_opl_mod_text(tdf, output=False):
    """
    Generate a OPL .mod string from a TicDat object for diagnostic purposes
    :param tdf: A TicDatFactory defining the schema
    :param output: An optional boolean used to signify output schemas
    ...
    def get_table_as_mod_text(tdf, tbn, output=False):
        rtn = ''
    ...
    for t in dict_tables:
        rtn += get_table_as_mod_text(input_tdf, t, output)
    return rtn
