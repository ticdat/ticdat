...
from ticdat import TicDatFactory, freeze_me, GurobiCallBackAndLog
...
def solve(dat, out, err, progressFunction = lambda progDict : True):
    ...
    useless = [n for n in dat.sites if not any(canAssign(y,n) for y in dat.sites) and
                                             dat.sites[n]["demand"] == 0]
    if useless:
        err.write("The following sites have no demand, and serve as the center point for any assignments.\n")
    ...
    with GurobiCallBackAndLog(dat.parameters["guLogFileName"]["value"]
